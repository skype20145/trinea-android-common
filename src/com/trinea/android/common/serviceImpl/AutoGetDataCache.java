...
public class AutoGetDataCache<K, V> extends SimpleCache<K, V> {
    ...
    /**
     * 用{@link #get(Object)}获取key对应的值，并自动获取新数据进行缓存
     * 
     * @param key 待获取值的key
     * @param keyList key list，按照{@link #autoCacheNewDataBack(Object, List, int)}中规则向前缓存，按照
     * {@link #autoCacheNewDataForward(Object, List, int)}中规则向后缓存，为空表示不进行缓存
     * @return
     */
    ...
    /**
     * 使用{@link #getAsync(Object)}获取key对应的值，并自动获取新数据进行缓存
     * 
     * @param key 待获取值的key
     * @param keyList key list，按照{@link #autoCacheNewDataBack(Object, List, int)}中规则向前缓存，按照
     * {@link #autoCacheNewDataForward(Object, List, int)}中规则向后缓存，为空表示不进行缓存
     * @return
     */
    public CacheObject<V> getAsync(K key, List<K> keyList) {
        if (key == null) {
            return null;
        }
    ...
    /**
     * 获取key对应值，若不在缓存中直接返回null
     * <ul>
     * <li>若key不在缓存中，返回null</li>
     ...
     * @return
     */
    public CacheObject<V> getAsync(K key) {
        return super.get(key);
    }

    /**
     * 获取key对应值，若不存在实时获取并等待数据获取成功将其返回
     * <ul>
     * <li>若该key为null，返回null</li>
     * <li>若key已在缓存中，返回该key对应的值</li>
     * <li>若key不在缓存中，会自动调用其{@link OnGetDataListener#onGetData(Object)}方法等待数据获取完成将其返回，getData为null时返回null</li>
     * </ul>
     * 
     * @param key
     * @return
     ...
     * <li>线程池大小为{@link #DEFAULT_THREAD_POOL_SIZE}</li>
     * </ul>
     * 
